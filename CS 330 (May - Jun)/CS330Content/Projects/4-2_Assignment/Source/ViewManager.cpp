...
void ViewManager::ProcessKeyboardEvents()
{
	...
	//
	// camera upward movement when pressing q
	if (glfwGetKey(m_pWindow, GLFW_KEY_Q) == GLFW_PRESS)
	{
		g_pCamera->ProcessKeyboard(UP, gDeltaTime);
	}
	
	// camera downward movement when pressing e
...
void ViewManager::PrepareSceneView()
{
 ...
 // This method is automatically called from GLFW whenever
 // scroll wheel is utilized.
 
void ViewManager::Mouse_Scroll_Callback(GLFWwindow* window, double xOffset, double yOffset)
{
	if (NULL != g_pCamera)
	{
		// modify camera speed based on scroll delta
		g_pCamera->MovementSpeed += static_cast<float>(yOffset) * 0.5f;

		// I am Speed.
		if (g_pCamera->MovementSpeed < 0.1f) 	// Scroll speed floor cap
			g_pCamera->MovementSpeed = 0.1f;
		if (g_pCamera->MovementSpeed > 20.0f) 	// Scroll speed ceiling cap
